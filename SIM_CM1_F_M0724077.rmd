---
title: "**Analisis Performa Revenue dan Pola Rental Film pada Database Sakila**"
...
df_actor_roi %>%
  ...
  knitr::kable(
    format = "html", 
    escape = FALSE, # Agar simbol $ tidak merusak header
    digits = 2,
    col.names = c("Nama Aktor", "Jml Film", "Jml Rental", "Total Revenue (USD)", "Revenue/Film (USD)"),
...
- Terdapat **variasi signifikan** dalam revenue antar aktor meski semua berada di top 20 — menunjukkan bahwa popularitas film (dan bukan sekadar jumlah film) menjadi penentu utama ROI.
- Aktor dengan **revenue per film tinggi** (warna biru tua pada bubble chart) adalah aktor yang paling efisien: sedikit film tapi menghasilkan banyak transaksi rental.
- Tidak terdapat korelasi linear sempurna antara jumlah film dan total revenue — beberapa aktor dengan **lebih sedikit film** justru menghasilkan revenue lebih tinggi, mengindikasikan bahwa **kualitas dan popularitas film** lebih berpengaruh daripada kuantitas.
- **Implikasi strategis:** kontrak dengan aktor ber-ROI tinggi per film harus diprioritaskan untuk menjaga efisiensi portofolio konten Sakila.
</div>
...
df_rfm_summary %>%
  ...
  row_spec(which(df_rfm_summary$segment == "Champions"),
           bold = TRUE, background = "#d5f5e3", color = "#1e8449") %>%
  row_spec(which(df_rfm_summary$segment == "At Risk"),
...
**Dari scatter plot RFM**, terlihat jelas pemisahan visual antara Champions (kiri-atas: recency baru, frekuensi tinggi) dan Lost (kanan-bawah), mengkonfirmasi validitas segmentasi yang dihasilkan.
</div>
...
### 🎭 Actor ROI
- Terdapat **variasi besar** dalam kontribusi revenue antar aktor — kuantitas film bukan satu-satunya penentu ROI.
- Aktor dengan **revenue per film tinggi** adalah aset strategis utama yang harus diprioritaskan dalam kontrak ke depan.
